...
class ValueOptimizer:
    ...
    def remove_dot_zero(value):
        try:
            if ValueOptimizer.is_float(value):
                value = str(value)
    ...
    @staticmethod
    def magnitude_alignment(original_value, value_start_index, question, final_value, col_data_type, col_values):
        value_ext = question[value_start_index: value_start_index + len(original_value) + 2]
        value_float_flag = ValueOptimizer.is_float(final_value)
        magnitude_flag = False
        unit = ""
        for char in "百千万亿":
            if char in value_ext:
                magnitude_flag = True
                unit = char
                break
        col_float_flag = True
        for value in col_values:
            if ValueOptimizer.is_float(value) is False:
                col_float_flag = False
                break
        col_type_flag = True if col_data_type == "real" else False
        if value_float_flag and magnitude_flag and col_float_flag and col_type_flag:
            col_values = list(map(lambda x: float(x), col_values))
            final_value = float(final_value)
            mean_col_value = np.mean(col_values)
            factor = mean_col_value / final_value
            if 10 < factor < 999 and unit == "百":
                factor = 100
            elif 100 < factor < 9999 and unit == "千":
                factor = 1000
            elif 1000 < factor < 99999 and unit in "万亿":
                factor = 10000
            elif factor < 1000:
                factor = 1
            else:
                # 拉平数量级
                mag_diff = (len(str(int(mean_col_value))) - len(str(int(final_value))))
                factor = 10**mag_diff
            final_value = str(int(float(final_value) * factor))
            return final_value
        else:
            return final_value
...
class Trainer:
    def __init__(self, data_dir, model_name, epochs=1, batch_size=64, base_batch_size=32, max_len=120, part=1., seed=1234, debug_mode=False):
        ...
        if os.path.exists(self.data_dir):
            self.train_data_path = os.path.join(self.data_dir, "nl2sql_train_20190618/train.json")
            self.train_table_path = os.path.join(self.data_dir, "nl2sql_train_20190618/train.tables.json")
            self.valid_data_path = os.path.join(self.data_dir, "nl2sql_val_20190618/val.json")
            self.valid_table_path = os.path.join(self.data_dir, "nl2sql_val_20190618/val.tables.json")
            self.test_data_path = os.path.join(self.data_dir, "nl2sql_test_20190618/test.json")
            self.test_table_path = os.path.join(self.data_dir, "nl2sql_test_20190618/test.tables.json")
            ...
            kaggle_input_dir = "../input/nl2sql/nl2sql_data/nl2sql_data/"
            self.train_data_path = os.path.join(kaggle_input_dir, "nl2sql_train_20190618/train.json")
            self.train_table_path = os.path.join(kaggle_input_dir, "nl2sql_train_20190618/train.tables.json")
            self.valid_data_path = os.path.join(kaggle_input_dir, "nl2sql_val_20190618/val.json")
            self.valid_table_path = os.path.join(kaggle_input_dir, "nl2sql_val_20190618/val.tables.json")
            self.test_data_path = os.path.join(kaggle_input_dir, "nl2sql_test_20190618/test.json")
            self.test_table_path = os.path.join(kaggle_input_dir, "nl2sql_test_20190618/test.tables.json")
    ...
    def create_dataloader(self):
        ...
        total_valid_list = [valid_conc_tokens, valid_tag_masks, valid_sel_masks, valid_con_masks, valid_type_masks, valid_attention_masks, valid_connection_labels, valid_agg_labels, valid_tag_labels, valid_con_num_labels, valid_type_labels, valid_cls_index_list, valid_header_question_list, valid_header_table_id_list, valid_header_masks, valid_question_masks, valid_subheader_cls_list, valid_subheader_masks, valid_sel_num_labels, valid_where_num_labels, valid_op_labels, valid_value_masks, order_list]
        valid_conc_tokens, valid_tag_masks, valid_sel_masks, valid_con_masks, valid_type_masks, valid_attention_masks, valid_connection_labels, valid_agg_labels, valid_tag_labels, valid_con_num_labels, valid_type_labels, valid_cls_index_list, valid_header_question_list, valid_header_table_id_list, valid_header_masks, valid_question_masks, valid_subheader_cls_list, valid_subheader_masks, valid_sel_num_labels, valid_where_num_labels, valid_op_labels, valid_value_masks, order_list = zip(*sorted(zip(*total_valid_list), key=lambda x: np.sum(np.array(x[0]) != 0), reverse=True))
    ...
    def determine_num(self, prob_list, num_prob, num_limit):
        best_num = 0
        best_joint_prob = -float('inf')
        for num in range(1, num_limit + 1):
            tmp_prob_list = prob_list[:num]
            tmp_prob_list = list(map(lambda x: x["prob"], tmp_prob_list))
            tmp_num_prob = num_prob[num]
            joint_prob = tmp_num_prob + np.mean(tmp_prob_list)
    ...
    def test(self, do_evaluate=True, do_test=True):
        ...
        if do_test:
            ...
            self.evaluate(logits_lists, cls_index_list, labels_lists, test_question_list, test_question_token_list, test_table_id_list, test_sample_index_list, test_sql_list, test_table_dict, test_header_question_list, test_header_table_id_list, total_probs_list, do_test=True)
