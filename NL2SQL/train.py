...
class Trainer:
    def __init__(self, data_dir, model_name, epochs=4, batch_size=64, base_batch_size=32, part=1., seed=1234, debug_mode=False):
        ...
        if os.path.exists(self.data_dir):
            ...
            kaggle_input_dir = "../input/nl2sql_data/nl2sql_data/"
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                sel_masks = batch_data[2]   # 至少要有一个？ 否则 continue？
                con_masks = batch_data[3]   # 至少要有一个？ 否则 continue？
                ...
                if True:
                    ...
                    for j, valid_batch_data in enumerate(valid_loader):
                        ...
                        connection_labels = valid_batch_data[6].to('cpu').numpy().tolist()
                        agg_labels = valid_batch_data[7].to('cpu').numpy().tolist()
                        tag_labels = valid_batch_data[8].to('cpu').numpy().tolist()
                        con_num_labels = valid_batch_data[9].to('cpu').numpy().tolist()
                        type_labels = valid_batch_data[10].to('cpu').numpy().tolist()
                        cls_indices = valid_batch_data[11].to('cpu').numpy().tolist()
                    ...
                    score = 0
                    print("epoch: %d duration: %d min \n" % (epoch, int((time.time() - train_start_time) / 60)))
                    print(eval_result)
                    ...
                    if not self.debug_mode:
                        ...
                        model_name = "model2/model_%s_%d_%d_%d_%dmin_%dmin_%.4f.bin" % (self.model_name, self.seed, epoch + 1, stage, train_duration, valid_duration, score)
