...
class BertNeuralNet(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # 输出层
        self.linear_out = nn.Linear(dense_size * 2, 1)
        self.linear_aux_out = nn.Linear(dense_size * 2, 5)
        self.linear_identity_out = nn.Linear(dense_size, 9)
        self.linear_np_out = nn.Linear(dense_size * 2, 4)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        # 拼接
        bert_hidden = self.linear1(bert_output)
        hidden = torch.cat((bert_hidden, bert_output_gate), 1)
...
class Trainer:
    def __init__(self, data_dir, model_name, epochs=4, batch_size=64, base_batch_size=32, part=1., seed=1234, debug_mode=False):
        ...
        if os.path.exists(self.data_dir):
            ...
            self.val_data_path = os.path.join(self.data_dir, "nl2sql_val_20190618/val.json")
            self.val_table_path = os.path.join(self.data_dir, "nl2sql_val_20190618/val.tables.json")
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                loss = target_loss * 0.5 + aux_loss * 0.167 + identity_loss * 0.167 + np_loss * 0.167
