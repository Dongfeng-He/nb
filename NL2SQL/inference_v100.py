    ...
    def forward(self, input_ids, attention_mask, all_masks, cls_index_list, train_dependencies=None):
        ...
        if train_dependencies:
            ...
            if torch.cuda.is_available():
                ...
                tag_output = torch.zeros(batch_size, seq_len, hidden_size, dtype=torch.float32)
...
class Predictor:
    ...
    def process_test_sample(self, sample, table_dict, bert_tokenizer):
        ...
        for col in range(len(table_header_list)):
            ...
            attention_masks.append(attention_mask)
            cls_index_list.append(header_cls_index)
        return type_masks, attention_masks, cls_index_list, conc_tokens
    ...
    def create_dataloader(self):
        ...
        for sample in test_data_list:
            ...
            test_cls_index_list.extend(processed_result[2])
            test_conc_tokens.extend(processed_result[3])
        ...
        test_dataset = data.TensorDataset(torch.tensor(test_type_masks, dtype=torch.long),
        ...
        # 返回训练数据
        return valid_loader, valid_question_list, valid_table_id_list, valid_sample_index_list, valid_sql_list, valid_table_dict, test_loader, test_question_list, test_table_id_list, test_sample_index_list, test_table_dict
    ...
    def evaluate(self, logits_lists, cls_index_list, labels_lists, question_list, table_id_list, sample_index_list, correct_sql_list, table_dict):
        ...
        for i in range(len(sample_index_list)):
                    ...
                    for i in range(len(value_start_index_list)):
                        ...
                        if op != 2:  # 不是 =，不能搜索，能比大小的应该就是数字
                            ...
                            if value_str not in col_values:
                                best_value = ValueOptimizer.select_best_matched_value_from_candidates(candidate_value_set, col_values)
                                if len(best_value) > 0:
            ...
            print("")
        acc = matched_num / len(sample_index_list)
        print("acc", acc)
        ...
        type_dict = {0: "sel", 1: "con", 2: "none"}
        """
        1. 把正确的sql传进来，甚至不需要label了（label用来计算各部分的正确率）
        2. 按照 sample index list 把所有的结果都group了
        3. 把table_dict 传进来
        """
        """
        tag_pred = []
    ...
    def generate_result(self, logits_lists, cls_index_list, question_list, table_id_list, sample_index_list, table_dict):
        [tag_logits_list, agg_logits_list, connection_logits_list, con_num_logits_list, type_logits_list] = logits_lists
        sql_dict = {"agg": [], "cond_conn_op": None, "sel": [], "conds": []}
        sql_list = []
        for i in range(len(sample_index_list)):
    ...
    def inference(self, do_evaluate=True, do_test=False):
        ...
        # 加载 dataloader
        valid_loader, valid_question_list, valid_table_id_list, valid_sample_index_list, valid_sql_list, valid_table_dict, test_loader, test_question_list, test_table_id_list, test_sample_index_list, test_table_dict = self.create_dataloader()
        # 训练
        self.seed_everything()
        ...
        if do_evaluate:
            ...
            for j, valid_batch_data in enumerate(valid_loader):
                ...
                if torch.cuda.is_available():
                    ...
                    sel_masks = sel_masks.to(self.device)
                    con_masks = con_masks.to(self.device)
                    type_masks = type_masks.to(self.device)
                    attention_masks = attention_masks.to(self.device)
                    connection_labels = connection_labels.to(self.device)
                    agg_labels = agg_labels.to(self.device)
                    tag_labels = tag_labels.to(self.device)
                    con_num_labels = con_num_labels.to(self.device)
                    type_labels = type_labels.to(self.device)
                    cls_indices = cls_indices.to(self.device)
            ...
            eval_result, tag_acc = self.evaluate(logits_lists, cls_index_list, labels_lists, valid_question_list, valid_table_id_list, valid_sample_index_list, valid_sql_list, valid_table_dict)
        if do_test:
            # 开始生成测试结果
            tag_logits_list = []
            agg_logits_list = []
            ...
            self.generate_result(logits_lists, cls_index_list, test_question_list, test_table_id_list, test_sample_index_list, test_table_dict)
