...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        for i, column in enumerate(self.identity_list):
            ...
            pn_label_bool = train_df_copy["target"] & (~train_df_copy[column])
            nn_label_bool = ~train_df_copy["target"] & (~train_df_copy[column])
            np_identity_weight[:, i * 4 + 0] = np.where(pp_label_bool > 0, math.log2(self.weight_dict["pp_%s" % column]), 1)
            np_identity_weight[:, i * 4 + 1] = np.where(np_label_bool > 0, math.log2(self.weight_dict["np_%s" % column]), 1)
            np_identity_weight[:, i * 4 + 2] = np.where(pn_label_bool > 0, math.log2(self.weight_dict["pn_%s" % column]), 1)
            np_identity_weight[:, i * 4 + 3] = np.where(nn_label_bool > 0, math.log2(self.weight_dict["nn_%s" % column]), 1)
        # target weight
        for column in self.identity_list + ["target"]:
...
print("train33_np_identity_task.py")
