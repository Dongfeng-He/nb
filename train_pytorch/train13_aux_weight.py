...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        target_weight += self.train_df["target"] * 2
        target_weight += np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * 2
        if False:
            ...
            target_weight += (~self.train_df["target"]) * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * 2
            target_weight += self.train_df["target"] * np.where((~self.train_df[self.identity_list]).sum(axis=1) > 0, 1, 0) * 2
