...
class NeuralNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat([result, aux_result, identity_result, np_result, np_identity_result], 1)
        return out
...
class Trainer:
    ...
    def create_dataloader(self):
        ...
        train_y_tensor = torch.tensor(np.hstack([train_label[:, np.newaxis], train_type_labels, train_identity_type_labels, train_np_labels, train_np_identity_labels]), dtype=torch.float32)
        valid_y_tensor = torch.tensor(np.hstack([valid_label[:, np.newaxis], valid_type_labels, valid_identity_type_labels, valid_np_labels, valid_np_identity_labels]), dtype=torch.float32)
    ...
    def custom_loss(self, y_pred, y_batch, epoch, target_weight=1., aux_weight=1., identity_weight=1., np_weight=1., np_identity_weight=1.):
        ...
        np_identity_pred = y_pred[:, 19: 55]
        np_identity_true = y_batch[:, 19: 55]
        if epoch > 9:
        ...
        return target_loss, aux_loss, identity_loss, np_loss, np_identity_loss
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_data in train_loader:
                ...
                y_pred = model(x_batch)
                target_loss, aux_loss, identity_loss, np_loss, np_identity_loss = self.custom_loss(y_pred, y_batch, epoch, target_weight_batch, aux_weight_batch, identity_weight_batch, np_weight_batch, np_identity_weight_batch)
                loss = target_loss + aux_loss + identity_loss + np_loss + np_identity_loss
...
print("train36_np_and_identity_task.py")
