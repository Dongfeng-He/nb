...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        if False:
            ...
            target_weight += (~self.train_df["target"]) * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * math.pow(self.weight_dict["np"], 0.5) / 4
            target_weight += self.train_df["target"] * np.where(self.train_df[self.identity_list].sum(axis=1) == 0, 1, 0) * math.pow(self.weight_dict["pn"], 0.5) / 4
...
print("train19_correct_weight.py")
