...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        if False:
            ...
            target_weight += (~self.train_df["target"]) * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * self.weight_dict["np"] / 5
            target_weight += self.train_df["target"] * np.where(self.train_df[self.identity_list].sum(axis=1) == 0, 1, 0) * self.weight_dict["pn"] / 5
            target_weight += self.train_df["target"] * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * self.weight_dict["pp"] / 5
...
print("train25_target_new_weight.py")
