...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        if False:
            ...
            target_weight += (~self.train_df["target"]) * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * self.weight_dict["np"]
            target_weight += self.train_df["target"] * np.where(self.train_df[self.identity_list].sum(axis=1) == 0, 1, 0) * self.weight_dict["pn"]
            target_weight += self.train_df["target"] * np.where(self.train_df[self.identity_list].sum(axis=1) > 0, 1, 0) * self.weight_dict["pp"]
...
print("train23_target_new_weight.py")
