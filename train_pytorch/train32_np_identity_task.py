...
class NeuralNet(nn.Module):
    def __init__(self, embedding_matrix):
        ...
        self.linear_np_identity_out = nn.Linear(dense_size, 36)
    ...
    def forward(self, x):
        ...
        np_identity_result = self.linear_np_identity_out(hidden)
        out = torch.cat([result, aux_result, identity_result, np_identity_result], 1)
        return out
...
class Trainer:
    ...
    def create_dataloader(self):
        ...
        train_y_tensor = torch.tensor(np.hstack([train_label[:, np.newaxis], train_type_labels, train_identity_type_labels, train_np_identity_labels]), dtype=torch.float32)
        valid_y_tensor = torch.tensor(np.hstack([valid_label[:, np.newaxis], valid_type_labels, valid_identity_type_labels, valid_np_identity_labels]), dtype=torch.float32)
    ...
    def cal_sample_weights(self):
        ...
        pn_label_bool = train_df_copy["target"] & np.where((train_df_copy[self.identity_list]).sum(axis=1) == 0, True, False)
        nn_label_bool = ~train_df_copy["target"] & np.where((train_df_copy[self.identity_list]).sum(axis=1) == 0, True, False)
        np_weight[:, 0] = np.where(pp_label_bool > 0, self.weight_dict["pp"], 1)
        np_weight[:, 1] = np.where(np_label_bool > 0, self.weight_dict["np"], 1)
        np_weight[:, 2] = np.where(pn_label_bool > 0, self.weight_dict["pn"], 1)
        np_weight[:, 3] = np.where(nn_label_bool > 0, self.weight_dict["nn"], 1)
        for i, column in enumerate(self.identity_list):
            pp_label_bool = train_df_copy["target"] & train_df_copy[column]
            np_label_bool = ~train_df_copy["target"] & train_df_copy[column]
            pn_label_bool = train_df_copy["target"] & (~train_df_copy[column])
            nn_label_bool = ~train_df_copy["target"] & (~train_df_copy[column])
            np_identity_weight[:, i * 4 + 0] = np.where(pp_label_bool > 0, math.pow(self.weight_dict["pp_%s" % column], 0.5), 1)
            np_identity_weight[:, i * 4 + 1] = np.where(np_label_bool > 0, math.pow(self.weight_dict["np_%s" % column], 0.5), 1)
            np_identity_weight[:, i * 4 + 2] = np.where(pn_label_bool > 0, math.pow(self.weight_dict["pn_%s" % column], 0.5), 1)
            np_identity_weight[:, i * 4 + 3] = np.where(nn_label_bool > 0, math.pow(self.weight_dict["nn_%s" % column], 0.5), 1)
        # target weight
        for column in self.identity_list + ["target"]:
    ...
    def custom_loss(self, y_pred, y_batch, epoch, target_weight=1., aux_weight=1., identity_weight=1., np_identity_weight=1.):
        ...
        np_identity_pred = y_pred[:, 15: 51]
        np_identity_true = y_batch[:, 15: 51]
        if epoch > 9:
        ...
        if epoch > 9:
            np_identity_loss = FocalLoss()(np_identity_pred, np_identity_true)
        else:
            np_identity_loss = nn.BCEWithLogitsLoss(reduction="none")(np_identity_pred, np_identity_true)
        np_identity_loss = torch.mean(np_identity_loss * np_identity_weight)
        return target_loss, aux_loss, identity_loss, np_identity_loss
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_data in train_loader:
                ...
                y_pred = model(x_batch)
                target_loss, aux_loss, identity_loss, np_identity_loss = self.custom_loss(y_pred, y_batch, epoch, target_weight_batch, aux_weight_batch, identity_weight_batch, np_identity_weight_batch)
                loss = target_loss + aux_loss + identity_loss + np_identity_loss
...
print("train32_np_identity_task.py")
