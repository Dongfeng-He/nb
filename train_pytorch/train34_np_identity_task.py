...
class Trainer:
    ...
    def cal_sample_weights(self):
        ...
        for i, column in enumerate(self.identity_list):
            ...
            pn_label_bool = train_df_copy["target"] & (~train_df_copy[column])
            nn_label_bool = ~train_df_copy["target"] & (~train_df_copy[column])
            np_identity_weight[:, i * 4 + 0] = np.where(pp_label_bool > 0, 1, 1)
            np_identity_weight[:, i * 4 + 1] = np.where(np_label_bool > 0, 1, 1)
            np_identity_weight[:, i * 4 + 2] = np.where(pn_label_bool > 0, 1, 1)
            np_identity_weight[:, i * 4 + 3] = np.where(nn_label_bool > 0, 1, 1)
        # target weight
        for column in self.identity_list + ["target"]:
...
print("train34_np_identity_task.py")
