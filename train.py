...
class Trainer:
    def __init__(self, data_dir):
        ...
        self.train_df = pd.read_csv(os.path.join(self.data_dir, "train.csv")).head(10000)
        self.test_df = pd.read_csv(os.path.join(self.data_dir, "test.csv")).head(10000)
    ...
    def create_train_data(self):
        ...
        # 划分训练集和验证集
        train_len = int(len(train_tokens) * self.split_ratio)
        valid_tokens = train_tokens[train_len:]
        valid_label = train_label[train_len:]
        valid_type_labels = train_type_labels[train_len:]
        train_tokens = train_tokens[:train_len]
        train_label = train_label[:train_len]
        train_type_labels = train_type_labels[:train_len]
        # 数据集
    ...
    def build_model(self, emb_weights):
        ...
        output1 = Bidirectional(GRU(16, return_sequences=True))(token_emb)
        output2 = Bidirectional(LSTM(16, return_sequences=True))(token_emb)
        output1 = GlobalMaxPooling1D()(output1)
        output2 = GlobalMaxPooling1D()(output2)
        # 拼接
        output = concatenate([output1, output2])
        # 全连接层
        output = hidden_layer(output, 16, "he_normal", "relu")
        output = hidden_layer(output, 16, "he_normal", "relu")
        # 输出层
